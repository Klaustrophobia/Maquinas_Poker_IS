  ...
  try {
    ...
    if (!correo || !contraseña) {
      return applyCorsHeaders(
        req,
        NextResponse.json({ error: "Correo y contraseña son requeridos" }, { status: 400 }),
        "POST,OPTIONS"
      );
    }
    ...
    const message = error instanceof Error ? error.message : "Error al procesar la solicitud de login";
    return applyCorsHeaders(
      req,
