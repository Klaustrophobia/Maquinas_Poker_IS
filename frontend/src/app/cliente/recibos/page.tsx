...
import { FileText, Search, X, Eye, Printer, Filter, Calendar, User, DollarSign, Package, Mail, Wrench, Download } from "lucide-react";
...
// Modal de Detalle con colores actualizados
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden animate-scale-in">
        {/* Header con gradiente rojo vino */}
        <div className="bg-gradient-to-r from-red-800 to-blue-900 p-6 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-red-200">Información completa del recibo</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
          {/* Información General - Tarjeta Azul */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl p-5 border border-blue-200 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-gray-700 font-bold">Total de Máquinas:</span>
                  <span className="font-bold text-blue-600 text-lg">{lote.cantidad_recibos}</span>
                </div>
              </div>
            </div>

            {/* Información del Cliente - Tarjeta Verde */}
          ...
          {/* Resumen Financiero - Tarjeta Púrpura */}
          <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-6 mb-8 border border-purple-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="text-center bg-white rounded-lg p-4 shadow-sm border border-green-100">
                <div className={`text-2xl font-bold ${lote.total < 0 ? 'text-red-600' : 'text-green-600'}`}>
              ...
              <div className="text-center bg-white rounded-lg p-4 shadow-sm border border-purple-100">
                ...
                <div className="text-sm text-gray-600 font-bold">Máquinas</div>
              </div>
            </div>
          </div>

          {/* Recibos Individuales - Tarjeta Café Claro */}
            ...
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl border border-amber-200 overflow-hidden shadow-sm">
              <div className="bg-gradient-to-r from-amber-100 to-orange-100 px-6 py-4 border-b border-amber-200">
                <h3 className="font-bold text-gray-900 flex items-center gap-3">
                  ...
                  Recibos Individuales por Máquina ({lote.recibos.length})
                </h3>
              </div>
              <div className="divide-y divide-amber-100">
                {lote.recibos.map((recibo, index) => (
                  <div key={recibo.id} className="p-6 hover:bg-amber-50/50 transition-colors group">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-sm text-gray-500 font-bold bg-amber-100 px-3 py-1 rounded-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div className="text-center bg-blue-50 rounded-lg p-3 border border-blue-200">
                        ...
                        <div className={`font-bold text-lg ${recibo.total < 0 ? 'text-red-600' : 'text-blue-600'}`}>
        ...
        <div className="bg-gradient-to-r from-gray-50 to-gray-100 px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
          <button
            onClick={onClose}
            className="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-all duration-200 font-bold"
          >
            Cerrar
          </button>
          <button
            ...
            className="px-6 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg hover:from-green-600 hover:to-emerald-700 transition-all duration-200 font-bold shadow-sm hover:shadow-md flex items-center gap-2"
          >
...
// Función de impresión (se mantiene igual)
...
export default function RecibosPage() {
  ...
  const estadisticas = {
    total: lotesRecibos.length,
    totalIngresos: lotesRecibos.reduce((sum, lote) => sum + lote.ingreso, 0),
    totalEgresos: lotesRecibos.reduce((sum, lote) => sum + lote.egreso, 0),
    totalNeto: lotesRecibos.reduce((sum, lote) => sum + lote.total, 0),
  };
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-600 font-bold">
                Consulta tu historial de pagos y estados de cuenta
              </p>
            </div>
            <div className="flex items-center gap-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent w-64 font-bold text-gray-900 bg-white"
                />
                {filtroBusqueda && (
                  <button
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-lg p-4 border border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">{formatMoneda(estadisticas.totalIngresos)}</p>
                </div>
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-lg p-4 border border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-red-600">{formatMoneda(estadisticas.totalEgresos)}</p>
                </div>
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-lg p-4 border border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={`text-2xl font-bold ${estadisticas.totalNeto < 0 ? 'text-red-600' : 'text-blue-600'}`}>
                    {formatMoneda(estadisticas.totalNeto)}
                  </p>
                </div>
                <DollarSign className={`w-8 h-8 ${estadisticas.totalNeto < 0 ? 'text-red-600' : 'text-blue-600'}`} />
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Lista de recibos con fondo decorativo */}
      <div className="bg-gradient-to-br from-white to-red-50/30 rounded-2xl shadow-lg border border-red-100/50 overflow-hidden relative">
        ...
        {/* Filtro activo */}
        {filtroBusqueda && (
          <div className="p-4 border-b border-red-100 bg-white/50 relative z-10">
            <div className="flex items-center justify-between">
        ...
        <div className="relative z-10">
          {loadingLotesRecibos ? (
            <div className="p-8">
              <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-sm p-6 animate-pulse border border-gray-100">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-sm overflow-hidden m-6 border border-gray-100">
                ...
                <tbody className="divide-y divide-gray-200">
                  {lotesRecibosFiltrados.map((lote) => (
                    <tr key={lote.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className={`text-sm font-bold ${lote.total < 0 ? 'text-red-600' : 'text-green-600'}`}>
                          {formatMoneda(lote.total)}
                        </div>
              ...
              <div className="grid grid-cols-2 gap-6 mb-4 seccion">
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between pt-1">
                    ...
                    <span className={`text-md font-bold ${reciboParaImprimir.total < 0 ? 'text-red-600' : 'text-black'}`}>
                      {formatMoneda(reciboParaImprimir.total)}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between border-b border-black pb-1">
                    ...
                    <span className={`text-md font-bold ${reciboParaImprimir.parte_empresa < 0 ? 'text-red-600' : 'text-black'}`}>
                      {formatMoneda(reciboParaImprimir.parte_empresa)}
                    </span>
                  </div>
                  <div className="flex justify-between pt-1">
                    <span className="text-md font-bold text-black">Cliente (40%):</span>
                    <span className={`text-md font-bold ${reciboParaImprimir.parte_cliente < 0 ? 'text-red-600' : 'text-black'}`}>
                      {formatMoneda(reciboParaImprimir.parte_cliente)}
                    </span>
                  </div>
                </div>
              </div>
